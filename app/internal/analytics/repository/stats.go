...
func (r *StatsRepository) GetDashboard(ctx context.Context, ownerID int) (schema.DashboardStats, error) {
	var s schema.DashboardStats
...
func (r *StatsRepository) GetHouseStats(ctx context.Context, ownerID int) ([]schema.HouseStatsItem, error) {
	...
	var items []schema.HouseStatsItem
	for rows.Next() {
		var h schema.HouseStatsItem
...
func (r *StatsRepository) GetTopByViews(ctx context.Context, ownerID, limit int) ([]schema.TopHouseStats, error) {
	return r.getTop(ctx, ownerID, "view_count", limit)
}

func (r *StatsRepository) GetTopByLikes(ctx context.Context, ownerID, limit int) ([]schema.TopHouseStats, error) {
	return r.getTop(ctx, ownerID, "like_count", limit)
}

func (r *StatsRepository) getTop(ctx context.Context, ownerID int, column string, limit int) ([]schema.TopHouseStats, error) {
	query := fmt.Sprintf(`
	...
	var items []schema.TopHouseStats
	for rows.Next() {
		var h schema.TopHouseStats
...
func (r *StatsRepository) GetLikesOverTime(ctx context.Context, ownerID, days int) ([]schema.LikesOverTime, error) {
	rows, err := r.db.Query(ctx, `
	...
	var items []schema.LikesOverTime
	for rows.Next() {
		var l schema.LikesOverTime
...
func (r *StatsRepository) GetPriceDistribution(ctx context.Context, ownerID int) ([]schema.PriceDistribution, error) {
	rows, err := r.db.Query(ctx, `
		SELECT
			CASE
	...
	var items []schema.PriceDistribution
	for rows.Next() {
		var p schema.PriceDistribution
...
func (r *StatsRepository) GetHouseDetailStats(ctx context.Context, ownerID int, slug string) (schema.HouseDetailStats, error) {
	var s schema.HouseDetailStats
	...
	if s.RecentViewers == nil {
		s.RecentViewers = []schema.ViewerInfo{}
	}
	if s.RecentLikers == nil {
		s.RecentLikers = []schema.LikerInfo{}
	}
	if s.RecentBookings == nil {
		s.RecentBookings = []schema.BookingInfo{}
	}
	if s.ViewsPerDay == nil {
		s.ViewsPerDay = []schema.LikesOverTime{}
	}
	if s.LikesPerDay == nil {
		s.LikesPerDay = []schema.LikesOverTime{}
	}
