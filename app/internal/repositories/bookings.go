...
func (r *BookingRepository) GetByID(ctx context.Context, id int) (schemas.BookingResponse, error) {
	return r.scanBooking(ctx, "b.id=$1", id)
}

func (r *BookingRepository) scanBooking(ctx context.Context, where string, arg interface{}) (schemas.BookingResponse, error) {
	var b schemas.BookingResponse
...
func (r *BookingRepository) GetUserBookings(ctx context.Context, userID int) ([]schemas.BookingResponse, error) {
	return r.queryBookings(ctx, "b.user_id=$1", userID)
}

func (r *BookingRepository) GetOwnerBookings(ctx context.Context, ownerID int) ([]schemas.BookingResponse, error) {
	return r.queryBookings(ctx, "h.owner_id=$1", ownerID)
}

func (r *BookingRepository) queryBookings(ctx context.Context, where string, arg interface{}) ([]schemas.BookingResponse, error) {
	query := fmt.Sprintf(`
		SELECT b.id, b.house_id, h.slug, h.name_en, h.name_kz, h.name_ru,
			   owner.id, CONCAT(owner.first_name, ' ', owner.last_name), owner.email, owner.phone_number,
			   guest.id, CONCAT(guest.first_name, ' ', guest.last_name), guest.email, guest.phone_number,
	...
	var items []schemas.BookingResponse
	for rows.Next() {
		var b schemas.BookingResponse
		var sd, ed, createdAt, updatedAt time.Time
		var ownerPhone, guestPhone *string
...
func (r *BookingRepository) GetUserActiveBooking(ctx context.Context, houseID, userID int) (*schemas.HouseBooking, error) {
	var b schemas.HouseBooking
	var sd, ed time.Time
	err := r.db.QueryRow(ctx, `
