...
.PHONY: help up down build restart logs ps app postgres clean prune install mode test test-v test-cover \
        ...
        bench bench-mixed bench-db cache-off cache-on air-off air-on pprof
...
help:
	...
	@echo "  make air-off                   🚫 Restart app as prod-like binary (no air/hot-reload) for benching"
	@echo "  make air-on                    🔁 Restart app with air (hot-reload)"
